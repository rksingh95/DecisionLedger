...
class HTTPDAIClient(BaseDAIClient):
    ...
    async def query(self, filters: QueryFilter) -> list[DecisionRecord]:
        ...
        if filters.from_timestamp:
            params["from_timestamp"] = filters.from_timestamp.isoformat()
        if filters.to_timestamp:
            params["to_timestamp"] = filters.to_timestamp.isoformat()
        if filters.outcome:
            params["outcome"] = filters.outcome
        if filters.exception_applied is not None:
            params["exception_applied"] = str(filters.exception_applied).lower()
        if filters.override_applied is not None:
            params["override_applied"] = str(filters.override_applied).lower()
        if filters.cursor:
    ...
    async def get_latest_hash(self) -> str:
        ...
        try:
            ...
            return str(resp.json().get("hash", GENESIS_HASH))
        except Exception as exc:
...
class SQLiteDAIClient(BaseDAIClient):
    ...
    async def query(self, filters: QueryFilter) -> list[DecisionRecord]:
        ...
        if filters.to_timestamp:
            clauses.append("decision_timestamp <= ?")
            params.append(filters.to_timestamp.isoformat())
        if filters.cursor:
            clauses.append("decision_id > ?")
